...
export async function apiLoadFicha(id: string): Promise<Ficha | null> {
  const res = await fetch(`/api/fichas/${encodeURIComponent(id)}`);
...
export async function apiDeleteFicha(id: string): Promise<void> {
  const res = await fetch(`/api/fichas/${encodeURIComponent(id)}`, { method: 'DELETE' });
  await assertOk(res);
}
...
export async function apiUploadAvatar(
  ...
  const res = await fetch(`/api/avatar/${encodeURIComponent(id)}`, {
    method: 'POST',
    body: form,
  });
  await assertOk(res);
...
export async function apiAvatarSemFundo(id: string): Promise<string | null> {
  const res = await fetch(`/api/avatar-sem-fundo/${encodeURIComponent(id)}`);

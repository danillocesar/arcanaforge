...
function VitalBar({ editMode = false, onToggleEdit, onOpenMenu }: VitalBarProps) {
  ...
  // Class line: "<classe> <nível>" using first class, else total level.
  const firstClass = character.classes[0];
  const classLabel = firstClass?.name
    ? `${firstClass.name} ${firstClass.level}`
    : '';
  const deity = character.deity?.trim();

  return (
    <header className={`${styles.appHeader} ${editMode ? styles.editing : ''}`}>
